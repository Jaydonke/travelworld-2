...
class PhraseBuilder {
  ...
  async extractPhrasesFromSentence(sentence, domainTerms, keywords) {
    ...
    for (const term of domainTerms) {
      ...
      for (let i = 0; i <= words.length - termWords.length; i++) {
        ...
        if (match) {
          // Add the term itself only if it meets minimum word count
          if (termWords.length >= CONFIG.minPhraseWords) {
            phrases.add(term);
          }
          
          // Try to build expanded phrases (especially for single-word terms)
    ...
    for (const { pattern } of keywords.compoundPatterns) {
      if (sentence.toLowerCase().includes(pattern)) {
        // Only add if it meets minimum word count
        const patternWords = pattern.split(/\s+/);
        if (patternWords.length >= CONFIG.minPhraseWords) {
          phrases.add(pattern);
        }
      }
    }
  ...
  isValidPhrase(phrase) {
    ...
    // Check length constraints (至少2个单词，最多4个单词)
    if (phrase.length < 3 || words.length < CONFIG.minPhraseWords || words.length > CONFIG.maxPhraseWords) {
      return false;
    }
  ...
  const scoredArticles = allArticles
    .filter(article => {
      ...
      if (checkTime && currentArticle.publishedTime && article.publishedTime) {
        ...
        // Also check if the target article is still in the future (not yet published)
        const now = new Date();
        if (article.publishedTime > now) {
          return false;
        }
      }
