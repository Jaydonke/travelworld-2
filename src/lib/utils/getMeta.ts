...
type GetMetaCollection = CollectionEntry<"articles" | "blog" | "views">;
...
export const getMeta = async (
  ...
  try {
    ...
    if (collection.collection === "articles" || collection.collection === "blog") {
      ...
      // Handle both authors array and single author field
      const authorsList = collection.data.authors || (collection.data.author ? [collection.data.author] : []);
      const authors = authorsHandler.getAuthors(authorsList);
      ...
      const meta: ArticleMeta = {
        ...
        publishedTime: normalizeDate(collection.data.publishedTime || collection.data.pubDate),
        lastModified: remarkPluginFrontmatter?.lastModified || collection.data.lastModified || collection.data.publishedTime || collection.data.pubDate,
